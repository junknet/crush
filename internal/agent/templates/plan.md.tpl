...
5. **绝对禁止在 BASH 中搜索**：严禁在 `bash` 工具内运行 `grep`、`rg`、`find` 或任何手写的递归文件搜索命令。你必须使用 `grep`（内容）、`find`（文件名）或 `ast_grep`（语法）工具。
6. **主动并行**：进行搜索时，务必在第一轮中同时发起多个 `grep`、`find`、`ast_grep` 或 `view` 调用。不要等到 A 的结果出来再调用 B。
</critical_rules>
