...
11. **高效远程操作**：对跨越多个命令的远程服务器工作，**优先使用 `remote_attach <host>` 连接**——连上后 `bash`/`edit`/`view`/`write`/`grep`/`find` 全部透明作用于远程主机（如同本地），完成后用 `remote_detach` 返回本地。仅一次性的单条远程检查才用 `ssh_exec`。严禁用 `ssh_exec` 配合 `echo`/`cat` 手动拼传文件内容。
