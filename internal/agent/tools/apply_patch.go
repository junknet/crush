...
func applyHunks(content string, hunks []patchHunk, path string) (string, error) {
	...
	for hi, h := range hunks {
		...
		// On a leading-indentation-tolerant match (tier c) the file's actual
		// indentation differs from the hunk's. Reindent the added lines to the
		// file's indentation using a per-level translation map derived from the
		// matched window; if any added line's indent level was never observed
		// in the matched context we ABORT rather than guess.
		if tier == seekTierIndent {
			reindented, ok := reindentNewLines(fileLines[matchIdx:matchIdx+len(searchBlock)], searchBlock, newLines)
			if !ok {
				return "", fmt.Errorf(
					"%s hunk %d matched only under indentation tolerance, but an added line uses an indent level not present in the matched context; cannot place it safely",
					path, hi+1)
			}
			newLines = reindented
		}
...
// zero match we fall to the next tier. If all tiers miss we abort with a
// not-found error naming the hunk.
//
...
func seekUnique(fileLines, pattern []string, start int, eof bool, path string, hunkNo int) (int, seekTier, error) {
	...
	// Tier (c) leading-indentation-tolerant unique match: strip the leading
	// run of ' '/'\t' on BOTH sides and require the remainder to match
	// byte-for-byte (the leading-indent-tolerant, unique-match strategy from
	// edit.go's resolveLeadingIndent fuzzy tier).
	...
	return 0, 0, fmt.Errorf(
		"%s hunk %d context not found in the file. The ' '/'-' lines must match the current file contents (whitespace-tolerant). Re-read the file and rebuild the hunk",
		path, hunkNo)
}
